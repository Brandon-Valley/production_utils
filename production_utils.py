...
WRITE_APP_ID_TO_FILE__RUN_SILENT__VBS_PATH = os.path.dirname(
    os.path.abspath(__file__)) + '\write_app_id_to_file__run_silent.vbs'
...
def show_popup_on_uncaught_exception__if__is_production(func, is_production=True):
    ...
    # running sys.exit() from within a program should not raise an error
    except SystemExit:
        pass
    except:
...
if __name__ == '__main__':
    ...
    show_popup_on_uncaught_exception__if__is_production(exit_py, is_production=True)
